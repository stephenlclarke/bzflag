...
coverage:
	...
	@test_binary="$$(find .build -path '*BZFlagPackageTests.xctest/Contents/MacOS/BZFlagPackageTests' -type f -print -quit)"; \
	...
	"$(SWIFT_LLVM_COV)" export -format=lcov -instr-profile="$$profile" "$$test_binary" --sources Sources/BZFlagCore > coverage.lcov
